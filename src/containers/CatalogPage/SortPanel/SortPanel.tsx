...
interface IProps {
  ...
  params: { sortBy: string; filterText: string };
}
...
const SortPanel = ({
  sortBy: { menu, onClick, defaultSortLabel },
  total,
  searchBy,
  params,
}: IProps) => {
  ...
  const dropdownLabel = useMemo(() => {
    return (
      menu.find(({ value }) => value === defaultSortLabel)?.label ??
      defaultSortValue
    );
  }, [defaultSortLabel]);
    ...
    () => (
      <Menu>
        {menu.map(({ label, value }) => (
          <Menu.Item key={value} onClick={() => handleSortClick(value, label)}>
            {label}
          </Menu.Item>
        ))}
        ...
        <Row className={styles.rightBar}>
          <Col span={10} className={styles.searchBar}>
            <Input
              ...
              defaultValue={searchParams.current.filterText}
            />
          </Col>
          <Col>{total} Товаров</Col>
        </Row>
      </Col>
    </Row>
  );
