...
export class Request {
  ...
  async GET(url, options?: IGet) {
    const { urlToUse } = _createApiParams({ ...options, controllerPrefix: this.apiPrefix, url });
    const { data } = await ApiClient.get(urlToUse);
    return data;
  }

  async POST(url, body: any, options?: IPost) {
    const { urlToUse } = _createApiParams({ controllerPrefix: this.apiPrefix, url, ...options });
